...
def save_ycalc(I, Q, U, V, f, azi, filename):
    ...
    if not os.path.exists("hdf5/simulation"):
        os.makedirs("hdf5/simulation")

    with h5py.File(f"hdf5/simulation/{filename}.hdf5", "w") as file:
...
#%% Species and line absorption
    ws.abs_speciesSet(species=[f"O2-Z-68-{CENTRAL_LINE_FREQ-10e6}-{CENTRAL_LINE_FREQ+10e6}", "O2-PWR98", "H2O-PWR98"])
    ws.Touch(ws.abs_lines_per_species)
    ws.ReadXML(ws.abs_lines_per_species, "catalogue/abs_lines/abs_lines_per_species.xml")
    ws.Wigner6Init()
